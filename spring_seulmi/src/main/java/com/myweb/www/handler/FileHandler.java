...
@Slf4j
@Component // 사용자 클래스를 빈으로 등
public class FileHandler {
	
	private final String UP_DIR = "/Users/dkraika44/Spring/image";
		
	public List<FileVO> uploadFiles(MultipartFile[] files){
		List<FileVO> flist = new ArrayList<>();
		//FileVO 생성, 파일을 경로에 맞춰서 저장, 썸네일 저장
		//날짜를 폴더로 생성하여 그날 그날 업로드 파일을 관리
		LocalDate date = LocalDate.now(); //2024-01-18
		String today = date.toString();
		today = today.replace("-", File.separator); // \{window} /{mac}
		
		// /Users/dkraika44/Spring/image//2024//01//18
		...
		for(MultipartFile file : files) {
			...
			//실제 파일이 저장되려면 첫 경로부터 다 설정이 되어 있어야 함.
			///Users/dkraika44/Spring/image//2024//01//18//1111_apple.jpg
	...
	private boolean isImageFile(File storeFile) throws IOException {
		Tika tika = new Tika();
		String mimeType = tika.detect(storeFile); //type image/jpg
