...
public class FileSweeper {
	...
	public void fileSweeper() {
		...
		//저장소를 검색할 때 필요한 파일의 경로 리스트(실제로 존재해야 될 리스트)
		List<String> currFile = new ArrayList<String>();
		
		for(FileVO fvo : dbList) {
			String filePath = fvo.getSaveDir()+File.separator+fvo.getUuid();
			String fileName = fvo.getFileName();
			currFile.add(BASE_PATH+filePath+"_"+fileName);
			
			//이미지라면 썸네일 경로도 추가
			if(fvo.getFileType() >0) {
				currFile.add(BASE_PATH+filePath+"_th_"+fileName);
			}
